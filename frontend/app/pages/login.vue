<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8 bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700">
      <div>
        <div class="mx-auto h-12 w-12 text-blue-600 dark:text-blue-500 flex items-center justify-center bg-blue-100 dark:bg-blue-900/30 rounded-full">
          <svg xmlns="http://www.w3.org/-2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
          </svg>
        </div>
        <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900 dark:text-white">
          Sign in to Doc Intelligence
        </h2>
      ...
      <form class="mt-8 space-y-6" @submit.prevent="handleLogin">
        ...
        <div v-if="error" class="text-red-500 text-sm text-center bg-red-50 dark:bg-red-900/30 p-2 rounded">
          {{ error }}
        </div>
...
definePageMeta({
  layout: 'empty',
  auth: false // Indicate to middleware that this page doesn't require auth
})
...
const handleLogin = async () => {
  ...
  const { success, error: msg } = await auth.login(email.value, password.value)
