"""
vLLM OCR Agent implementation.

Implements BaseOCRAgent for vLLM server (LightOnOCR-2-1B).
"""
...
class VLLMOCRAgent(BaseOCRAgent):
    """
    vLLM OCR Agent for remote vLLM server.
    
    Supports: LightOnOCR-2-1B via vLLM server.
    """
    ...
    def process_image(
        ...
        try:
            ...
            # LightOnOCR models expect ONLY the image without text prompt
            payload = {
                "model": self.model_id,
                "messages": [
                    {
            ...
            if response.status_code != 200:
                ...
                return OCRResponse(
                    success=False,
                    error=f"vLLM server error: {error_detail}",
            ...
            if "choices" not in result or len(result["choices"]) == 0:
                return OCRResponse(
                    success=False,
                    error="Invalid vLLM response: Missing or empty 'choices' field",
                    error_type="api_error"
                )
            ...
            if "message" not in result["choices"][0] or "content" not in result["choices"][0]["message"]:
                return OCRResponse(
                    success=False,
                    error="Invalid vLLM response: Missing 'content' field",
                    error_type="api_error"
                )
            ...
            return OCRResponse(
                success=False,
                error=f"vLLM server is not running at {self.base_url}",
    ...
    def process_pdf_page(
        ...
        try:
            ...
            if response.status_code != 200:
                return OCRResponse(
                    success=False,
                    error=f"vLLM server error: HTTP {response.status_code}",
                    error_type="api_error"
                )
