"""
Configuration loader and validator for M.DocAI.
...
class Config:
    """
    Singleton configuration object for M.DocAI.
    ...
    def _apply_env_overrides(cls, config_data: Dict[str, Any]) -> None:
        ...
        if poe_api_key:
            if 'split' not in config_data:
                config_data['split'] = {}
            config_data['split']['poe_api_key'] = poe_api_key
    ...
    def split_config(self) -> Dict[str, Any]:
        """
        Get document splitting configuration.
        
        Returns:
            Dictionary containing split settings (model, poe_api_key, max_categories, timeout, retry_attempts)
        """
        return self._config_data.get('split', {})
