"""
Workflow orchestrator — graph-aware executor for Dr.Vision pipelines.
...
async def _run_parse(node: Dict[str, Any], ctx: NodeContext, *, parse_formatting: bool) -> Dict[str, Any]:
    ...
    model_id, provider = TierConfig.get_parser_model_spec(node["tier"])
    if provider:
        ocr_agent = AgentFactory.create_ocr_agent({"model_id": model_id, "provider": provider, "base_url": "", "max_tokens": 4096, "temperature": 0.2, "top_p": 0.9})
    else:
...
async def _run_classify(node: Dict[str, Any], ctx: NodeContext) -> Dict[str, Any]:
    ...
    model_id, provider = TierConfig.get_classifier_llm_model_spec(node["tier"])
    llm_agent = AgentFactory.create_llm_agent(model_id, provider=provider, config=ctx.config if not provider else None)
...
async def _run_extract(node: Dict[str, Any], ctx: NodeContext) -> Dict[str, Any]:
    ...
    extractor_model, provider = TierConfig.get_extractor_model_spec(node["tier"])
    llm_agent = AgentFactory.create_llm_agent(extractor_model, provider=provider, config=ctx.config if not provider else None)
...
async def _run_split(node: Dict[str, Any], ctx: NodeContext) -> Dict[str, Any]:
    ...
    splitter_model, provider = TierConfig.get_splitter_model_spec(node["tier"])
    vlm_agent = AgentFactory.create_vlm_agent(splitter_model, provider=provider, config=ctx.config if not provider else None)
